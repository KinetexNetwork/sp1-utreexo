...
pub fn get_input_leaf_hashes_new(file_path: &str) -> HashMap<TxIn, BitcoinNodeHash> {
...
pub fn get_block_heights(data_path: &str) -> Result<Vec<u64>, Box<dyn Error>> {
...
pub fn write_input_leaf_hashes(input_leaf_hashes: &HashMap<TxIn, BitcoinNodeHash>, tx_count: u64) {
    let output_path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("processed-acc-data")
        .join(format!("block-{}txs", tx_count));
